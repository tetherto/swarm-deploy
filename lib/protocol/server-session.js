...
class ServerSession {
  ...
  _touch() {
    ...
    this.timer = this.scheduler.setTimeout(() => {
      ...
      this._queue(() =>
        this._failClosed(new SwarmDeployError(ERRORS.UPLOAD_IDLE_TIMEOUT, 'Upload idle timeout'))
      )
    }, this.idleTimeout)
  ...
  _send(index, value) {
    ...
    if (!message || message.send(value) === false)
      throw protocolError('Unable to send protocol response')
  }
  ...
  _sendBitmapPages(offerValue, verified) {
  ...
  async _rejectOffer(code, reason) {
    this._send(STATUS, { transferId: this.transferId, code, reason })
  ...
  async _handleOffer(value) {
    try {
      ...
      this._send(STATUS, { transferId: this.transferId, code: STATUS_CODE.ACCEPT })
      this._sendBitmapPages(value, snapshot.verified)
      this._send(READY, { transferId: this.transferId })
      this.state = 'READY'
      this._touch()
    } catch (err) {
  ...
  async _handleChunk(value) {
    try {
      ...
      this._send(CHUNK_ACK, { transferId: this.transferId, index: value.index })
      this._touch()
    } catch (err) {
      await this._failClosed(err)
    }
  ...
  async _handleFinish(value) {
    try {
      ...
      await this.commitStore.commit(session, { retentionManager: this.retentionManager })
      await this.sessionStore.retireCommitted(this.transferId)
      this._send(RESULT, { transferId: this.transferId, code: RESULT_COMMITTED })
      await this._terminal()
    } catch (err) {
