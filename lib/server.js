...
const { prepareStorageRecovery, recoverStorage } = require('./storage/recovery')
const {
  RetentionManager,
...
function assertNonnegativeSafeUint(value, name) {
...
function eventFailureReason(err) {
      ...
      if (this.allowlistPath) {
        ...
        this.allowlistWatcher = new AllowlistWatcher({
          ...
          onReload: (keys) => {
            ...
            return this.reloadAllowlist(keys)
          },
          onFailure: ({ reason }) => {
            this._emitSafe('allowlist', {
              status: 'failed',
    ...
    this.closePromise = (async () => {
      ...
      if (this.listenPromise) {
        try {
          ...
          if (err?.code !== ERRORS.ABORTED) errors.push(err)
        }
      }
      try {
        await this._dispose()
