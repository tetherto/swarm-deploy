...
function serializeByRoot(locks, root, operation) {
  const previous = locks.get(root) || Promise.resolve()
  const result = previous.then(operation, operation)
  const tracked = result.catch(() => {})
  locks.set(root, tracked)
  return result.finally(() => {
    if (locks.get(root) === tracked) locks.delete(root)
  })
}
...
class RetentionManager {
  ...
  async run(options = {}) {
    return serializeByRoot(retentionLocks, this.layout.root, async () => {
