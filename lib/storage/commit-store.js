...
class CommitStore {
  ...
  async _removeManagedFinalOrAbsent(record) {
    ...
    await withSafeDirectoryIdentity(this.layout.root, this.storage, async () => {
      ...
      if (!stat.isFile() && !stat.isSymbolicLink()) {
        throw storageError('Unsafe managed final path')
      }
  ...
  async commit(session, { retentionManager = null } = {}) {
    assertSession(session)
    if (retentionManager !== null && typeof retentionManager.run !== 'function') {
      throw storageError('Invalid retention manager')
    }
    const key = `${this.layout.root}\0${session.id}\0${session.name}`
    const previous = commitLocks.get(key) || Promise.resolve()
    const result = previous.then(
      () => this._commit(session, retentionManager),
      () => this._commit(session, retentionManager)
    )
    const tracked = result.catch(() => {})
    commitLocks.set(key, tracked)
    try {
      return await result
    } finally {
      if (commitLocks.get(key) === tracked) commitLocks.delete(key)
    }
  }

  async _commit(session, retentionManager) {
    await this._assertLayout()
    const record = this._recordFromSession(session)
    const stagingPath = this._stagingPath(record.transferId)
    const finalPath = this._finalPath(record.name)

    if (retentionManager) await retentionManager.run({ incomingBytes: record.size })
    ...
    try {
      ...
      if (retentionManager) await retentionManager.run()
      return record
    } catch (err) {
  ...
  async purge(record) {
    ...
    await this._removeManagedFinalOrAbsent(record)
    await this._removeFile(this._recordPath(record.transferId), this.layout.commits)
    return true
  }
}
