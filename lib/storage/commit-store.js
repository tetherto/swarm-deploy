...
const {
  ...
  assertCommitRecord: assertRecord,
  readCommitJournal,
  isHex,
  fileIdentity,
  identitiesEqual
} = require('./commit-journal')
...
class CommitStore {
  ...
  async _markAttemptAborting(id, expectedAttemptId) {
    const journal = await this._readJournal(id)
  ...
  async _removeAbortedPublication(record, expectedIdentity) {
    ...
    if (errors.length) throw new AggregateError(errors, 'Unable to roll back revoked publication')
  }

  async _abortAttempt(record, expectedIdentity, attemptId, revoked) {
    try {
  ...
  async retryAbortedAttempt(transferId, sessionStore) {
    ...
    return withRootLease(this.layout.root, async () => {
      ...
      if (!session || session.id !== id || session.state !== 'verified') {
        throw storageError('Missing verified session for revoked attempt retry')
      }
      assertSession(session)
      if (
        session.name !== journal.record.name ||
        session.size !== journal.record.size ||
        toHex(session.digest) !== journal.record.sha256 ||
        fingerprint(session.ownerKey) !== journal.record.uploaderFingerprint
      ) {
      ...
      return this._discardJournal(id, journal.attemptId)
    })
  }
  ...
  async recoverJournal(id, sessionStore, { isAuthorized = null } = {}) {
    await this._assertLayout()
