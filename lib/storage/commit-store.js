...
const { writeAtomic, readJson, MAX_SESSION_METADATA_BYTES } = require('./atomic-file')
...
class CommitStore {
  ...
  async _writeJournal(record) {
    ...
    await writeAtomic(journalPath, b4a.from(JSON.stringify(journal)), this.storage)
  }

  async _discardJournal(id) {
    return this._removeFile(this._journalPath(id), this.layout.journals)
  }

  async _linkStaging(stagingPath, finalPath, expectedIdentity) {
    return withSafeDirectoryIdentity(this.layout.root, this.storage, () =>
      withSafeDirectoryIdentity(this.layout.staging, this.storage, async () => {
        ...
        if (staging.dev !== expectedIdentity.dev || staging.ino !== expectedIdentity.ino) {
  ...
  _recordFromSession(session) {
    ...
    const record = {
      ...
      uploaderFingerprint: toHex(crypto.createHash('sha256').update(session.ownerKey).digest()),
  ...
  async commit(session) {
    ...
    await this._writeJournal(record)
    let linked = false
    try {
      await this._linkStaging(stagingPath, finalPath, stagingDigest.identity)
      linked = true
      ...
      await this._discardJournal(record.transferId)
      return record
    } catch (err) {
      if (!linked && err?.code !== 'EEXIST') {
        linked = await this._matchesRecord(finalPath, this.layout.root, record)
      }
      if (linked) throw err
  ...
  async _readJournal(id) {
    ...
    if (!stat) return null
    const journal = await readJson(journalPath, this.storage, MAX_COMMIT_METADATA_BYTES)
    if (
      journal.version !== JOURNAL_VERSION ||
      journal.state !== 'committing' ||
      journal.transferId !== id
    ) {
      throw storageError('Invalid commit journal')
    }
    const record = assertRecord(journal.record)
    if (record.transferId !== id) throw storageError('Commit journal ID mismatch')
    return record
  }

  async _verifiedResumable(id, record) {
    const sessionPath = this._sessionPath(id)
    const stagingPath = this._stagingPath(id)
    if (!(await this._safeFileOrAbsent(sessionPath, this.layout.sessions))) return false
    if (!(await this._safeFileOrAbsent(stagingPath, this.layout.staging))) return false

    const metadata = await readJson(sessionPath, this.storage, MAX_SESSION_METADATA_BYTES)
    if (
      metadata.version !== 1 ||
      ...
      metadata.digest !== record.sha256
    ) {
      return false
    }
    return this._matchesRecord(stagingPath, this.layout.staging, record)
  }

  async recoverJournal(id) {
    await this._assertLayout()
    const record = await this._readJournal(id)
    ...
    if (await this._verifiedResumable(id, record)) {
      await this._discardJournal(id)
