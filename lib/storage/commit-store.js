...
class CommitStore {
  ...
  _assertSessionMatchesRecord(session, record, message) {
    assertSession(session)
    if (
  ...
  async _inspectJournalPublication(journal) {
  ...
  async _validateAttemptLeftovers(id, journal, sessionStore) {
    const { record, sourceStagingIdentity } = journal
  ...
  async _cleanupCommittedAttempt(id, journal, sessionStore) {
