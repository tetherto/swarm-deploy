...
class MetadataFormatError extends SwarmDeployError {
...
async function readJson(filePath, storage = fs.promises, maxBytes = MAX_SESSION_METADATA_BYTES) {
  ...
  return withSafeDirectoryIdentity(directory, storage, async () => {
    ...
    try {
      ...
      let parsed
      try {
        parsed = JSON.parse(b4a.toString(await readAll(handle, stat.size), 'utf8'))
      } catch (err) {
        throw new MetadataFormatError(`Malformed metadata file: ${filePath}`, err)
      }
      ...
      if (err instanceof SwarmDeployError || err instanceof MetadataFormatError) throw err
      throw err
    } finally {
