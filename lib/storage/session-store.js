...
const { MAX_CHUNK_BYTES, MAX_CHUNK_COUNT } = require('../protocol/constants')
...
const { readCommitJournal, fileIdentity, identitiesEqual } = require('./commit-journal')
...
class SessionStore {
  ...
  async _removeFile(filePath, directory, ensureSynced = false) {
    let removed = false
  ...
  async _deleteSession(session, reason = 'delete') {
  ...
  async _journalOwnsStaging(id, staging) {
    let journal
    try {
  ...
  async _assertDiskReserve(incomingBytes) {
  ...
  async deleteUnauthorized(isAuthorized) {
    return this._run(async () => {
      this._assertReady()
