...
class SessionStore {
  ...
  async _hasCommitJournal(id) {
    const journalPath = path.join(this.layout.journals, `${id}.json`)
    try {
      await assertSafeFile(journalPath, this.storage)
      return true
    } catch (err) {
      if (err.cause?.code === 'ENOENT') return false
      throw err
    }
  }
  ...
  async init() {
    return this._run(async () => {
      ...
      for (const name of stagingNames) {
        ...
        if (!match || !(await this._hasCommitJournal(match[1]))) {
