...
class Client extends EventEmitter {
  ...
  _reportResult(result) {
    const details = { name: result.name, status: result.status, final: true }
  ...
  async _upload(inputPath) {
    ...
    const batch = { status: failed ? 'FAILED' : 'COMMITTED', results, skipped: selection.skipped }
    this._emitSafe('result', { status: batch.status, final: true, files: results.length })
    return batch
  }
